...
WHERE expires > UTC_TIMESTAMP() ORDER BY id DESC LIMIT 10`
	...
	// database connection.
	// What's more, even if rows.Next() encounter error during loop, it only stops the loop and save error in rows.Error()
	// So it's important to use if err = rows.Err(); err != nil to decide whether there is an error or not.
	for rows.Next() {
		...
		// Why don't we use ErrNoRecord here, just like what we have done in GET()
		// It is because that for multiple lines query, the result of no line is normal behavior, not an error
		// We don't need to know the detailed information about data storing model to deal with the error.
		if err != nil {
			return nil, err
		}
